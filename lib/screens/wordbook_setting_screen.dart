...
class _WordBookSettingScreenState extends State<WordBookSettingScreen> {
  ...
  Future<List<WordPreviewModel>?> _getMyWordList({required int wordbookId}) async {
    try {
      // 1) API 호출 (단어장 ID 전달)
      final result = await MywordbooksService.getMyWordList(wordbookId: wordbookId);

      // 2) 응답 처리
      return await _handleApiResponse<List<WordPreviewModel>> (
          context, result, mounted: mounted, customErrorContext: '단어 목록 조회'
      );
    } catch (e) {
      rethrow;
    }
  ...
  Future<bool> _deleteMyWordBookWord({required int wordbookId, required int wordId}) async {
    try {
      // 1) API 호출 (단어장 ID, 단어 ID 전달)
      final result = await MywordbooksService.deleteMyWordBookWord(wordbookId: wordbookId, wordId: wordId);

      // 2) 응답 처리 
      await _handleApiResponse<dynamic> (
        context, result, mounted: mounted, customErrorContext: '단어 삭제'
      );

      // 3) 단어 삭제 후 리스트 업데이트
      final updatedList = await _getMyWordList(wordbookId: wordbookId); // await 키워드를 사용하면 List 타입, 사용하지 않으면 Future 타입
  ...
  Future<bool> _deleteMyWordBook({required int wordbookId}) async {
    try {
      // 1) API 호출 (단어장 ID 전달)
      final result = await MywordbooksService.deleteMyWordBook(wordbookId: wordbookId);

      // 2) 응답 처리
      await _handleApiResponse<dynamic> (
          context, result, mounted: mounted, customErrorContext: '단어장 삭제'
      );
  ...
  Future<bool> _putMyWordBookTitle({required int wordbookId, required String newTitle}) async {
    try {
      // 1) API 호출 (단어장 ID 전달)
      final result = await MywordbooksService.putMyWordBookTitle(wordbookId: wordbookId, newTitle: newTitle);
      
      // 2) 응답 처리
      await _handleApiResponse<dynamic> (
          context, result, mounted: mounted, customErrorContext: '단어장 수정'
      );
            ...
            child: ElevatedButton(
              onPressed: () async {
                if (newName.isNotEmpty && newName != widget.setName) {
                  try {
                    // 1) API 호출
                    await _putMyWordBookTitle(wordbookId: widget.wordbookId, newTitle: newName);

                    // 2) 성공 시 처리 이전 화면으로 돌아감
                    Navigator.pop(context, { "action": "updated", "newName": newName});
                  } catch (e) {
                  ...
                  // 동일한 제목일 경우 경고
                  ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(content: Text("현재 제목과 동일합니다.")),
                  );
                }
              },
            ...
            child: Center(
              child: Column(
                children: [
                  Row(
                    children: [
                      Text("현재 제목", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16),),
                      SizedBox(width: 45,),
                      Text("${widget.setName}", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16),),
                    ]
                  ),
                  SizedBox(height: 10,),
                  Row(
                      children: [
                        Text("수정할 제목", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16),),
                        SizedBox(width: 30,),
                        Container(
                          ...
                          child: TextFieldLineWidget(
                            hint: "수정할 제목을 입력해주세요.",
                            fieldHeight: 35,
                            fieldWidth: MediaQuery.of(context).size.width * 0.6,
                            lineThickness: 6,
                            obscureText: false,
                  ...
                  FutureBuilder(
                      ...
                      builder: (context, snapshot) {
                        ...
                        return Row(
                          children: [
                            Text('총 ${wordList.length} 단어', style: TextStyle(color: Colors.black26)),
                          ],
                        );
                      }),
                  SizedBox(height: 10),
                                ...
                                children: [
                                  ...
                                  children: [
                                    ...
                                    Padding(
                                      ...
                                      child: ElevatedButton(
                                        onPressed: () async {
                                          print('삭제 버튼 클릭 - wordbookId: ${widget.wordbookId}, wordId: ${word.wordId}');

                                          try {
                                            await _deleteMyWordBookWord(wordbookId: widget.wordbookId, wordId: word.wordId!);
                                            // 단어장에 단어가 없을 때 (다 삭제됐을 때) 단어장도 같이 삭제
                                            final updatedMyWordBookWordList = await _getMyWordList(wordbookId: widget.wordbookId);
                                            if (updatedMyWordBookWordList == null || updatedMyWordBookWordList.isEmpty) {
...
Widget buildAsyncStatusWidget (BuildContext context, AsyncSnapshot snapshot)
{
  if (snapshot.connectionState == ConnectionState.waiting) {
    return Row(
      children: [
        Text('로딩 중...', style: TextStyle(color: Colors.black26)),
      ],
    );
  }
  if (snapshot.hasError) {
    return Row(
      children: [
        Text('에러 발생', style: TextStyle(color: Colors.red)),
      ],
    );
  }
